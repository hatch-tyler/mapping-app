...
test.describe('Map Interaction', () => {
  ...
  test('should show layer manager', async ({ page }) => {
    // The map toolbar exposes Layers / Legend / Basemaps panels (icon
    // buttons with title-based accessible names); the Layers panel is open
    // by default. Assert the toolbar controls are present.
    await expect(page.getByRole('button', { name: /layers/i }).first()).toBeVisible({ timeout: 10000 });
    await expect(page.getByRole('button', { name: /basemap/i }).first()).toBeVisible();
  });
